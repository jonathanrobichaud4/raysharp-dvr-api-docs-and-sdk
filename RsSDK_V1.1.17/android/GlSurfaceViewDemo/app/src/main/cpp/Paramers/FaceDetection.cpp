...
{
	FaceDetection* pThis = (FaceDetection*)userParam;
	RSNet_SoleIntHumanFace_t* faceD = (RSNet_SoleIntHumanFace_t*)data;
	pThis->m_vec.push_back(*faceD);
}

std::string FaceDetection::QueryAll(bool bDefault)
{
	//请求数据
...
std::string FaceDetection::QueryAll(std::string strData)
{
	//请求数据
	std::string strRet = "err";
	...
	int defaultKind = obj["defaultKind"].asUInt();
	...
	param.ParamType = defaultKind;//MsgDefaultFaceFrontalView,MsgDefaultFaceMultiView
	param.Req = &req;
...
void FaceDetection::StructToJson(int n)
{
	m_obj.clear();
	m_obj["ch"] = n;
    ...
    m_obj["FaceType"] = m_vec[n].FaceType;
	m_obj["RollRange"] = m_vec[n].RollRange;
	m_obj["PitchRange"] = m_vec[n].PitchRange;
	m_obj["YawRange"] = m_vec[n].YawRange;
	m_obj["BlurStage"] = m_vec[n].BlurStage;
	m_obj["MinPixel"] = m_vec[n].MinPixel;
	m_obj["RuleKind"] = m_vec[n].RuleKind;
	m_obj["SnapNum"] = m_vec[n].SnapNum;
}

int FaceDetection::SaveAll(std::string strData)
{
	if(strData == "null"){
		return false;
	}
	...
	for(int i=0; i<obj.size(); i++)
	{
		...
		m_vec[i].FaceType = obj[i]["FaceType"].asInt();
		m_vec[i].RollRange = obj[i]["RollRange"].asInt();
		m_vec[i].PitchRange = obj[i]["PitchRange"].asInt();
		m_vec[i].YawRange = obj[i]["YawRange"].asInt();
		m_vec[i].BlurStage = obj[i]["BlurStage"].asInt();
