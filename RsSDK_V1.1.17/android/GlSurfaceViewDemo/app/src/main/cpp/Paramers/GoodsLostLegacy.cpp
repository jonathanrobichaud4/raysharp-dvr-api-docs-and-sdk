#include "StdAfx.h"
#include "GoodsLostLegacy.h"

static void RSNET_CALL AllDataCallback(void* data, void* userParam)
{
	GoodsLostLegacy* pThis = (GoodsLostLegacy*)userParam;
	RSNet_SoleGoodsLostLegacy_t* goodlostlegacy = (RSNet_SoleGoodsLostLegacy_t*)data;
	pThis->m_vec.push_back(*goodlostlegacy);
}

std::string GoodsLostLegacy::QueryAll(bool bDefault)
{
	//请求数据
	...
	param.ParamType = MsgParamGoodsLostLegacy;
	param.Req = &req;

	param.ListParamSize = sizeof(RSNet_SoleGoodsLostLegacy_t);
...
void GoodsLostLegacy::StructToJson(int n)
{
	m_obj.clear();
	m_obj["ch"] = n;
	...
	m_obj["DetonateTime"] = m_vec[n].DetonateTime;

	for(int i=0; i<8; ++i)
	{
		m_obj["RuleRectEnable"].append(m_vec[n].RuleRectEnable[i]);
	}
	
	for(int i = 0;i < 32;++i)
	{
		Json::Value val(Json::objectValue);
		val["x1"] = m_vec[n].RuleRect[i].x1 < 0 ? 0 : m_vec[n].RuleRect[i].x1;
		val["y1"] = m_vec[n].RuleRect[i].y1 < 0 ? 0 : m_vec[n].RuleRect[i].y1;
		val["x2"] = m_vec[n].RuleRect[i].x2 < 0 ? 0 : m_vec[n].RuleRect[i].x2;
		val["y2"] = m_vec[n].RuleRect[i].y2 < 0 ? 0 : m_vec[n].RuleRect[i].y2;
...
int GoodsLostLegacy::SaveAll(std::string strData)
{
	if(strData == "null"){
		return false;
	}
	...
	for(int i=0; i<obj.size(); i++)
	{
		...
		m_vec[i].DetonateTime = obj[i]["DetonateTime"].asInt();
		m_vec[i].StructSize = sizeof(RSNet_SoleGoodsLostLegacy_t);
	
		for(int j=0; j<obj[i]["RuleRectEnable"].size(); j++)
		{
			m_vec[i].RuleRectEnable[j] = obj[i]["RuleRectEnable"][j].asInt();
		}
		
		for(int j = 0;j < obj[i]["RuleRect"].size();++j)
		{
			m_vec[i].RuleRect[j].x1 = obj[i]["RuleRect"][j]["x1"].asInt();
			m_vec[i].RuleRect[j].y1 = obj[i]["RuleRect"][j]["y1"].asInt();
			m_vec[i].RuleRect[j].x2 = obj[i]["RuleRect"][j]["x2"].asInt();
			m_vec[i].RuleRect[j].y2 = obj[i]["RuleRect"][j]["y2"].asInt();
		}
