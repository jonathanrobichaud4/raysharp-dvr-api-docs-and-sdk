...
std::string PerimeterZone::QueryAll(bool bDefault)
{
	...
	param.ParamType = MsgParamPerimeter;
	param.Req = &req;
...
void PerimeterZone::StructToJson(int n)
{
	m_obj.clear();
	m_obj["ch"] = n;
...
int PerimeterZone::SaveAll(std::string strData)
{
	if(strData == "null"){
		return false;
	}
