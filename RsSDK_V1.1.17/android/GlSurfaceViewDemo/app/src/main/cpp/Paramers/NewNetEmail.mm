#include "StdAfx.h"
#include "NewNetEmail.h"

std::string NewNetEmail::QueryAll(bool bDefault)
{
	//ÇëÇóÊý¾Ý
	...
	memset(&m_EmailSet, '\0', sizeof(RSNet_NewEmailSet_t));
	m_EmailSet.StructSize = sizeof(RSNet_NewEmailSet_t);
	param.Param = &m_EmailSet;

	param.ParamSize = sizeof(RSNet_NewEmailSet_t);
	param.ParamType = MsgParamNewEmail;
...
void NewNetEmail::StructToJson()
{
	...
	m_obj["StreamType"] = m_EmailSet.StreamType;
	for(int i = 0;i < 3;++i)
	{
		m_obj["EnReceivePasswd"][i] = m_EmailSet.EnReceivePasswd[i];
	}
}


int NewNetEmail::SaveAll(std::string strData)
{	
	if(strData == "null"){
		return false;
	}
	...
	m_EmailSet.StreamType = obj["StreamType"].asInt();
	for(int i = 0;i < 3;++i)
	{
		m_EmailSet.EnReceivePasswd[i] = obj["EnReceivePasswd"][i].asInt();
	}
